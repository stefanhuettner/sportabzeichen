#!/usr/bin/env python3
"""Scan all Google Docs for plain-text URLs, make them clickable, and verify links."""

import json, re, sys, time, requests

# Load token
with open("/root/.openclaw/.google-tokens.json") as f:
    tokens = json.load(f)
access_token = tokens["access_token"]

headers = {
    "Authorization": f"Bearer {access_token}",
    "Content-Type": "application/json"
}

# Load docs from drive-links.json
with open("/root/clawd/projects/drive-links.json") as f:
    docs_list = json.load(f)

# Add extra docs
extra_docs = [
    {"name": "Preisstruktur Löwenstark", "docId": "1g9ufW_S7KOVhFa3Ur7yAEQiQZHjcpfmWlvVxXIqCdAI"},
    {"name": "Funnel-Übersicht", "docId": "1Zs7qdHoVtoO1ICCz9-ZSc3IKtLBVK9jVtsoqonlbnNw"},
    {"name": "Susannes Aufgaben", "docId": "1fMW3z1rfmsMIG-W0ShlbCMgzwUdeWJ8yOxUIq-fipdE"},
    {"name": "Alle Aufgaben & Ideen", "docId": "12aujMSWTrZjGNzOTVSDUSD8ZRHT7Y9rNqiEo_yW-nMs"},
]
docs_list.extend(extra_docs)

URL_PATTERN = re.compile(r'https?://[^\s\u00A0\u200B\u2028\u2029\n\r\t\]\)>}{\"\']+')

stats = {
    "docs_checked": 0,
    "docs_with_issues": 0,
    "total_urls_found": 0,
    "urls_fixed": 0,
    "urls_already_linked": 0,
    "mismatched_links": [],
    "broken_urls": [],
    "errors": [],
    "fixed_details": [],
}

all_unique_urls = set()

def find_urls_in_doc(doc):
    """Find all URLs in document body, return list of (url, startIndex, endIndex, has_link, link_url)."""
    results = []
    body = doc.get("body", {})
    
    def process_elements(elements):
        for elem in elements:
            if "paragraph" in elem:
                for pe in elem["paragraph"].get("elements", []):
                    if "textRun" in pe:
                        tr = pe["textRun"]
                        content = tr.get("content", "")
                        start = pe.get("startIndex", 0)
                        style = tr.get("textStyle", {})
                        link = style.get("link", {})
                        link_url = link.get("url", "")
                        
                        # Find URLs in the text content
                        for m in URL_PATTERN.finditer(content):
                            url = m.group(0).rstrip('.,;:!?)>')
                            url_start = start + m.start()
                            url_end = url_start + len(url)
                            has_link = bool(link_url)
                            results.append((url, url_start, url_end, has_link, link_url))
            
            if "table" in elem:
                for row in elem["table"].get("tableRows", []):
                    for cell in row.get("tableCells", []):
                        process_elements(cell.get("content", []))
            
            # Handle nested structures  
            if "tableOfContents" in elem:
                process_elements(elem["tableOfContents"].get("content", []))
    
    process_elements(body.get("content", []))
    return results

def fix_links_in_doc(doc_id, urls_to_fix):
    """Apply updateTextStyle to make plain-text URLs clickable."""
    if not urls_to_fix:
        return 0
    
    requests_list = []
    # Process in reverse order to maintain indices
    for url, start, end, _, _ in sorted(urls_to_fix, key=lambda x: x[1], reverse=True):
        requests_list.append({
            "updateTextStyle": {
                "range": {
                    "startIndex": start,
                    "endIndex": end
                },
                "textStyle": {
                    "link": {
                        "url": url
                    }
                },
                "fields": "link"
            }
        })
    
    # Batch update (max 100 per request)
    fixed = 0
    for i in range(0, len(requests_list), 100):
        batch = requests_list[i:i+100]
        resp = requests.post(
            f"https://docs.googleapis.com/v1/documents/{doc_id}:batchUpdate",
            headers=headers,
            json={"requests": batch}
        )
        if resp.status_code == 200:
            fixed += len(batch)
        else:
            stats["errors"].append(f"BatchUpdate failed for {doc_id}: {resp.status_code} - {resp.text[:200]}")
    
    return fixed

def check_url(url):
    """Check if a URL is reachable."""
    try:
        resp = requests.head(url, timeout=10, allow_redirects=True, 
                           headers={"User-Agent": "Mozilla/5.0"})
        return resp.status_code
    except requests.exceptions.SSLError:
        try:
            resp = requests.head(url, timeout=10, allow_redirects=True, verify=False,
                               headers={"User-Agent": "Mozilla/5.0"})
            return resp.status_code
        except:
            return -1
    except Exception as e:
        return -1

# Process each document
for doc_info in docs_list:
    doc_id = doc_info["docId"]
    doc_name = doc_info["name"]
    print(f"\n📄 Processing: {doc_name}")
    
    # Fetch document
    resp = requests.get(
        f"https://docs.googleapis.com/v1/documents/{doc_id}",
        headers=headers
    )
    
    if resp.status_code != 200:
        print(f"  ❌ Failed to fetch: {resp.status_code}")
        stats["errors"].append(f"Failed to fetch {doc_name} ({doc_id}): {resp.status_code}")
        continue
    
    doc = resp.json()
    stats["docs_checked"] += 1
    
    # Find URLs
    urls = find_urls_in_doc(doc)
    print(f"  Found {len(urls)} URLs")
    stats["total_urls_found"] += len(urls)
    
    # Separate: needs fixing vs already linked
    to_fix = []
    for url, start, end, has_link, link_url in urls:
        all_unique_urls.add(url)
        if not has_link:
            to_fix.append((url, start, end, has_link, link_url))
            print(f"  🔧 Needs link: {url[:80]}... [{start}:{end}]")
        else:
            stats["urls_already_linked"] += 1
            # Check mismatch
            if link_url and link_url != url and url not in link_url and link_url not in url:
                stats["mismatched_links"].append({
                    "doc": doc_name,
                    "displayed": url,
                    "linked_to": link_url
                })
                print(f"  ⚠️ Mismatch: displays '{url[:60]}' but links to '{link_url[:60]}'")
    
    # Fix unlinked URLs
    if to_fix:
        stats["docs_with_issues"] += 1
        fixed = fix_links_in_doc(doc_id, to_fix)
        stats["urls_fixed"] += fixed
        for url, start, end, _, _ in to_fix:
            stats["fixed_details"].append({"doc": doc_name, "url": url})
        print(f"  ✅ Fixed {fixed} links")
    
    time.sleep(0.3)  # Rate limiting

# Now check all unique URLs
print(f"\n\n🔍 Checking {len(all_unique_urls)} unique URLs...")
for url in sorted(all_unique_urls):
    # Skip Google Docs URLs (they require auth)
    if "docs.google.com" in url or "drive.google.com" in url or "forms.google.com" in url:
        continue
    status = check_url(url)
    if status not in (200, 301, 302, 303, 307, 308, 403, 405):
        # 403/405 often just block HEAD requests but work fine
        stats["broken_urls"].append({"url": url, "status": status})
        print(f"  ❌ {status}: {url}")
    else:
        print(f"  ✅ {status}: {url[:80]}")
    time.sleep(0.2)

# Output results
print("\n\n" + "="*60)
print("RESULTS")
print("="*60)
print(json.dumps(stats, indent=2, ensure_ascii=False))
